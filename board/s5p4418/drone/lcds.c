...
static struct nxp_lcd wsvga_ftl = {
	.width= 1024,
	.height = 600,
	.p_width = 153,
	.p_height = 86,
...
int drone_setup_lcd_by_id(int id)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(drone_lcd_config); i++) {
		if (drone_lcd_config[i].id == id) {
			lcd_idx = i;
			break;
		}
	}

	if (i >= ARRAY_SIZE(drone_lcd_config)) {
		/* NOT found */
		return -19;
	}

	return drone_lcd_config[i].id;
}

int drone_setup_lcd_by_name(char *str)
{
	char *delim;
	int i;
	...
	if (!strncasecmp("HDMI", str, 4)) {
		struct hdmi_config *cfg = &drone_hdmi_config[0];
		struct nxp_lcd *lcd;

		lcd_idx = ARRAY_SIZE(drone_lcd_config) - 1;
		lcd = drone_lcd_config[lcd_idx].lcd;

		for (i = 0; i < ARRAY_SIZE(drone_hdmi_config); i++, cfg++) {
			if (!strcasecmp(cfg->name, str)) {
				lcd->width = cfg->width;
				lcd->height = cfg->height;
				drone_lcd_config[lcd_idx].name = cfg->name;
				goto __ret;
			}
		}
	}
	...
	for (i = 0; i < ARRAY_SIZE(drone_lcd_config); i++) {
		if (!strcasecmp(drone_lcd_config[i].name, str)) {
			lcd_idx = i;
			break;
		}
	}
...
struct nxp_lcd *drone_get_lcd(void)
{
	return drone_lcd_config[lcd_idx].lcd;
}

const char *drone_get_lcd_name(void)
{
	return drone_lcd_config[lcd_idx].name;
}

enum lcd_format drone_get_lcd_format(void)
{
	return drone_lcd_config[lcd_idx].fmt;
}

int drone_get_lcd_density(void)
{
	return drone_lcd_config[lcd_idx].dpi;
}
